...
def send_message(message: str, video_filepath: Optional[str] = None) -> Tuple[str, Optional[Dict]]:
    ...
    model_message, emotion = ai_client.chat_respond(
    ...
    print(model_message, emotion)
    return model_message, emotion
...
def send_semantic_proactive(change_summary: str, image_filepath: Optional[str]) -> Tuple[str, Optional[Dict]]:
    """
...
class SaveAndSendWorker(QThread):
    ...
    # emotion_ready no longer used; kept for signal compatibility
    ...
    def run(self):
        try:
            ...
            # Use fast_send to minimize first-reply latency and avoid saving video buffers
            response, emotion = send_message(self.text, None)
            print("[SaveAndSendWorker] response ready.")
            # Emit main reply immediately without waiting for emotion analysis
            self.response_ready.emit(response, emotion)
            # Kick off separate emotion analysis request that won't block the UI
        except Exception as e:
...
class ProactiveMessageWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # Ensure we have recording enabled (video mode expected)
            video_tail = None
            try:
                # Ensure 4 seconds at 24fps (>=96 frames) with audio; saved under recordings/
            ...
            if video_tail and os.path.exists(video_tail):
                try:
                    ...
                    description = None
            # Build a minimal message for LLM chat
            user_msg = f"[Recent activity: {description}]" if description else "[no input...]"
            response, emotion = send_message(user_msg, video_tail)
